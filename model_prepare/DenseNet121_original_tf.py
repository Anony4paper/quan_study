...
def transition_block(x, reduction, name):
  ...
  x = tfmot.quantization.keras.quantize_annotate_layer(layers.BatchNormalization(
      axis=bn_axis, epsilon=1.001e-5, name=name + '_bn'), quantize_config=quantize_config)(
          x)
...
def conv_block(x, growth_rate, name):
  ...
  """
  bn_axis = 3 if backend.image_data_format() == 'channels_last' else 1
  x1 = tfmot.quantization.keras.quantize_annotate_layer(layers.BatchNormalization(
      axis=bn_axis, epsilon=1.001e-5, name=name + '_0_bn'), quantize_config=quantize_config)(
          x)
  ...
  x1 = layers.Conv2D(
      4 * growth_rate, 1, use_bias=False, name=name + '_1_conv')(
          x1)
  x1 = tfmot.quantization.keras.quantize_annotate_layer(layers.BatchNormalization(
      axis=bn_axis, epsilon=1.001e-5, name=name + '_1_bn'), quantize_config=quantize_config)(
          x1)
  x1 = layers.Activation('relu', name=name + '_1_relu')(x1)
  x1 = layers.Conv2D(
      growth_rate, 3, padding='same', use_bias=False, name=name + '_2_conv')(
          x1)
  x = tfmot.quantization.keras.quantize_annotate_layer(layers.Concatenate(axis=bn_axis, name=name + '_concat')
                                                       , quantize_config=quantize_config)([x, x1])
  return x
...
def DenseNet(
  ...
  x = tfmot.quantization.keras.quantize_annotate_layer(layers.BatchNormalization(
      axis=bn_axis, epsilon=1.001e-5, name='conv1/bn'), quantize_config=quantize_config)(
          x)
  ...
  x = tfmot.quantization.keras.quantize_annotate_layer(layers.BatchNormalization(axis=bn_axis, epsilon=1.001e-5, name='bn'), quantize_config=quantize_config)(x)
